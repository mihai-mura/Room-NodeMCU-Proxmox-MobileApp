...
import { Button, Platform, Pressable, StatusBar, StyleSheet, Text, View } from 'react-native';
...
export default function App() {
	...
	useEffect(() => {
		(async () => {
			...
			ws.send(JSON.stringify({ type: 'pushToken', token }));
		})();
	}, []);
	...
	useEffect(() => {
		ws.onmessage = (message) => {
			...
			} else if (payload.type === 'node-state') {
	...
	useEffect(() => {
		switch (state.state) {
			case 'offline':
				setTitle(`${state.name} is offline`);
				setTitleColor('#d3435c');
				break;
			case 'online':
				setTitle(`${state.name} is online`);
